...
- `planning/nest-contract.md` — wire contract with `proxai_nest` (authoritative).
- `~/Desktop/watermark_check.md` — watermark sync endpoint and structured 400 spec (backend coordination authority for `GET /v1/watermarks`).
- `src/` — implementation.
...
| File | Status | Notes |
|---|---|---|
| `nest-contract.md` | **CURRENT (UPDATED)** | Authoritative wire contract. Updated v2.1 → v2.2: added `GET /v1/watermarks` (§2.3), structured `watermark_regression` 400 body (§3.2 / §6.3), stable `host_id` derivation (§5.4), hysteresis `BUFFER_FULL` sentinel (§8.3). |
...
| `04_AGENT_CALL_RECORD.md` | **CURRENT (REVIEWED)** | Backend `AgentCallRecord` schema spec. Gateway ships raw bytes per `nest-contract.md`; this doc remains the right reference for backend parsers. Header-only review note added. |
...
- If you're looking for the current state of any feature, start with `README.md` (project root) and `src/`. The wire contract is `nest-contract.md`.
