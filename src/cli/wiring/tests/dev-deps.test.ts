...
import { requireDefined, requireRecord } from 'core/utils';
...
describe('buildDevServiceUnitPath', () => {
  test('darwin returns launchd plist path', () => {
  ...
  test('linux returns systemd unit path', () => {
  ...
  test('win32 returns scheduled task xml path', () => {
...
describe('verifyKeySimple', () => {
  test('creates HttpClient and returns success result', async () => {
  ...
  test('propagates failure result', async () => {
    __deps.createHttpClient = () => {
...
describe('writeDevConfigFull', () => {
  test('builds config and writes to file', async () => {
    ...
    const config = requireRecord(calls[0]?.config);
    const account = requireRecord(config.account);
    expect(account.apiKey).toBe('test-api-key');
    expect(account.userId).toBe('dev');
    expect(account.hostId).toBe('mock-uuid::dev');
  });
});

describe('registerDevHostIdFull', () => {
  test('derives dev host_id and registers via http client', async () => {
  ...
  test('surfaces already-bound result', async () => {
    __deps.readMachineUuid = async () => 'machine-xyz';
    __deps.deriveHostId = (uuid: string, userId: string) => `${uuid}::${userId}`;
    __deps.createHttpClient = () => {
...
describe('buildDevDeps', () => {
  test('returns correct dev command dependencies', async () => {
  ...
  test('clearAuthFailed targets the dev profile AUTH_FAILED sentinel', async () => {
...
describe('registerDevServiceUnit', () => {
  test('calls writeServiceUnit and ensureRegistered on darwin', async () => {
    __deps.defaultLaunchdPlistPath = () => '/tmp/mock.plist';
    __deps.devLaunchdLabel = () => 'mock-label';
    __deps.buildWatchdogServiceContext = () => null;
    let writeInput: WriteServiceUnitInput | null = null;
    ...
    expect(requireDefined<WriteServiceUnitInput>(writeInput).profileName).toBe('dev');
    expect(ensured).toBe(true);
  });

  test('installs dev watchdog when context is returned', async () => {
    const { mkdtemp, rm } = await import('node:fs/promises');
    const testDir = await mkdtemp(join(tmpdir(), 'proxai-dev-deps-test-'));
    const plistPath = join(testDir, 'watchdog.plist');

    try {
      __deps.defaultLaunchdPlistPath = () => '/tmp/mock.plist';
      __deps.devLaunchdLabel = () => 'mock-label';
      let writeUnitCalls = 0;
      __deps.writeServiceUnit = async () => {
        writeUnitCalls++;
      };
      let watchdogInstalled = false;
      const mockWatchdogManager: import('cli/watchdog-manager/types.ts').WatchdogManager = {
        install: async () => {
      ...
      __deps.buildWatchdogServiceContext = () => ({
        platform: 'darwin',
        watchdogUnitPaths: { plistPath },
        watchdogManager: mockWatchdogManager,
      });
      __deps.getServiceManager = () => createMockServiceManager();

      const deps = buildDevDeps();
      await deps.registerDevServiceUnit();

      expect(writeUnitCalls).toBe(1);
  ...
  test('__deps wrappers invoke the real underlying implementations', async () => {
    ...
    try {
      ...
      const watchdogCtx = __deps.buildWatchdogServiceContext(
        'darwin',
        '/bin/gateway',
        buildProfileContext('dev'),
      );
      expect(watchdogCtx).not.toBeNull();
      expect(watchdogCtx?.platform).toBe('darwin');
    } finally {
