...
test('insertReceipt + getReceipt round-trips all fields', () => {
  ...
  expect(requireDefined(stored).captureId).toBe(receipt.captureId);
  expect(requireDefined(stored).sourceApp).toBe(receipt.sourceApp);
  expect(requireDefined(stored).sourcePathHash).toBe(receipt.sourcePathHash);
  expect(requireDefined(stored).watermarkKind).toBe(receipt.watermarkKind);
  expect(requireDefined(stored).watermarkStart).toBe(receipt.watermarkStart);
  expect(requireDefined(stored).watermarkEnd).toBe(receipt.watermarkEnd);
  expect(requireDefined(stored).watermarkTable).toBe('threads');
  expect(requireDefined(stored).deliveredAt).toBe(receipt.deliveredAt);
  expect(requireDefined(stored).idempotentOnServer).toBe(true);
});
...
test('receipt with watermarkTable: null round-trips as null', () => {
  ...
  insertReceipt(db, receipt);
  const stored = requireDefined(getReceipt(db, receipt.captureId));
...
test('idempotentOnServer: false persists as false', () => {
  ...
  insertReceipt(db, receipt);
  expect(requireDefined(getReceipt(db, receipt.captureId)).idempotentOnServer).toBe(false);
});
