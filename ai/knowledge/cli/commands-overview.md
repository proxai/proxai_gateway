...
| `logs` | Record-centric view from `buffer.db` (`upload_receipts`, `upload_batches`, `resync_events`). Prod view: timestamp, source, prompt snippet, size. Dev view (dev mode or `--profile dev`): all receipt columns. Watch-mode by default; `--static` for one-shot. `--error` shows failures/quarantined/regression loops. |
