...
on Windows). Source: `src/core/io/fs/paths.ts:49-69`.

| File                | Writer                              | Clearer                     | Body                                                  |
| ------------------- | ----------------------------------- | --------------------------- | ----------------------------------------------------- |
| `PAUSED`            | `pause` CLI, `checkStaleBinary`     | `resume` CLI only           | reason string (free-form)                             |
...
`sentinelHandle(path)` in `core/io/fs/sentinel.ts:7-26` is the only writer
abstraction. It:
...
**Capture cycle** (`capture-cycle.ts:39-59`):
1. `AUTH_FAILED` — skip with reason `auth_failed`
2. `PAUSED` — skip with reason `paused`
3. `BUFFER_FULL` — skip with reason `buffer_full`

First hit short-circuits and emits `capture.cycle.skipped` (INFO).

**Drain cycle** (`drain-cycle.ts:29-37`):
1. `AUTH_FAILED` — skip
2. `PAUSED` — skip
3. **No `BUFFER_FULL` check.** Drain is the mechanism that *clears*
   `BUFFER_FULL`; gating on it would deadlock.

**Heartbeat cycle** (`heartbeat-cycle.ts:26-39`):
1. `PAUSED` — skip
2. **No `AUTH_FAILED` check.** Version checks and auto-upgrade must run
   even when auth is broken, otherwise a daemon with a revoked key would
   never self-heal.
...
- **Why `0o600`?** The `reason` body for `PAUSED` can include user-
  supplied text; the `detected_at` timestamps leak machine identity.
  Restricting to owner-read prevents shared-host snooping. Skipped on
...
`session-stopped-sentinel.ts:44-53`:
...
**Do not replicate this pattern to other sentinels.** `PAUSED` written
by stale-binary policy survives reboots intentionally — a stale binary
is stale until upgraded, regardless of reboot.

## Recovery edge cases

- **`PAUSED` from stale-binary**: cleared only by `resume` CLI, never by
  `upgrade`. Recovery is `upgrade` (which exits the process) +
  `resume` (next boot, after service manager restarts).
