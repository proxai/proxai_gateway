...
| `config` | Loads/writes `config.toml` once at daemon start. Resolves nest base URL from `dev-mode` sentinel. Computes defaults (`50 GiB` soft-pause, `365 d` retention). | The in-memory `GatewayConfig` (immutable for process lifetime). | Read once at daemon bootstrap; never re-read mid-run. |
...
| `rescue` | Runs the headless `rescue` command from periodic OS watchdog tasks. Computes rescue decisions (start/restart) based on heartbeat liveness, sentinel flags, and consecutive failure circuit breakers. | `rescue-ledger.json` schema + consecutive failures count, last rescue timestamp, and last observed heartbeat. | Not a daemon-loop module — runs out-of-process via watchdog scheduler. |
...
- **Sentinels** are the only cross-loop signal. Capture writes `BUFFER_FULL`; drain clears it. Drain writes `AUTH_FAILED`; the self-healing auth-recovery loop clears it on the first successful re-verify (and `setup new` / `dev setup` also clear it). See `ai/knowledge/services/sentinel-lifecycle.md` for the full table.
...
- `rescue.md` — ledger schema, rescue decision logic (start/restart/none), circuit-breaker consecutive failure increment rule, double-check comparison for wedge confirmation.
...
[source: src/services/{buffer,config,contract,http,polling,redaction,rescue,uploader,upgrade,uninstall}/index.ts; src/services/polling/daemon-loops.ts:26-41; src/services/polling/polling.constants.ts:5-9]
