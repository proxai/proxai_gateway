...
| `polling` | Three loops (capture / drain / heartbeat) under `Promise.all`. Five sentinel readers/writers (auth, buffer, pause, session-stopped, update-available). Source-poll dispatcher (worker thread or in-process fallback). | Sentinel files; per-loop in-memory cycle state passed only to callbacks. | Owns all daemon loops. |
...
- **Sentinels** are the only cross-loop signal. Capture writes `BUFFER_FULL`; drain clears it. Drain writes `AUTH_FAILED`; only `setup --force` clears it. Heartbeat writes `PAUSED` via stale-binary policy; only `resume` clears it. See `ai/knowledge/services/sentinel-lifecycle.md` for the full table.
