...
- Binary replacement must stage to `<path>.new` then `rename` over the path on **every** platform — never `Bun.write(binaryPath, bytes)` in place. `Bun.write` truncates the existing inode in place; modifying an already-executed binary in place breaks the running process and leaves a stale code-signing verdict cached against that inode, so on macOS the next exec is SIGKILLed (and on Linux the in-place write hits `ETXTBSY` while the daemon runs). The `rename` swaps in a fresh inode: the running daemon keeps the old inode until it exits, and the respawn execs a clean vnode. On Windows the running `.exe` is also locked, so `replaceBinary` leaves `<path>.new` for the service-manager wrapper to promote on restart (it returns `stagedSibling`); POSIX renames immediately and returns `stagedSibling: null`.
