...
- `markBatchDelivered` (insert receipt + delete batch) and `pruneBuffer` (delete old receipts, failed, quarantined + update metadata) each run inside one `db.transaction(...)`. Do not split them.
