...
- `AUTH_FAILED` is written only by `handleAuthError` in the drain cycle's upload path; it is cleared only through the setup reconfigure flow — `setup new` for prod or `dev setup` for the dev profile, both via `clearAuthFailedSentinel`. Do not add other writers or clearers.
