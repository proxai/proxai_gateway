...
- `PROXAI_GATEWAY_NEST_ENDPOINT` overrides the resolved nest base URL at module load. After `setup` writes `[backend]` to `config.toml`, the env var has no effect on a running daemon. To change the endpoint for a running install, edit `[backend]` or re-run `setup new`.
