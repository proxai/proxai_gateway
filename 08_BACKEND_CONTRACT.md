...
3. **`GET /v1/watermarks?host_id=<host_id>`** — authenticated. Returns the server's known per-`(user_id, host_id)` watermark cursors so a fresh gateway instance can seed its local cursor table after a `~/.proxai` wipe. Read-only; one row per `(source_app, source_path_hash, watermark_table)`. See §2.3.
...
### 2.1 Key verification (setup-time)
...
Authenticated by the **ingestion key** in the `X-API-Key` header. The header value is the full key string, format `<random>-<datestr>-<random>` (three hyphen-separated parts). The backend looks up the key by signature hash, validates `state: ACTIVE` and `type: INGESTION`, and on success returns the key metadata.
...
The gateway calls this exclusively during `proxai-gateway setup`: on 200, setup proceeds and writes config; on 403, setup aborts with an "ingestion key rejected" error; on 5xx/network errors, setup aborts with a transient-error message and the user can retry.
...
Authenticated via the user's INGESTION-type API key in the `X-API-Key` header. Same key format and lookup path as §2.1. The backend validates `state: ACTIVE` and `type: INGESTION`, and binds the request to the key's owner.

**Important:** the ingestion key is per-user, not per-host. A user may run the gateway on multiple machines, all with the same key. The `ApiKeyType` enum has only `PROVISIONING | SERVICE | INGESTION` — there is no `AGENT_GATEWAY` type. The gateway-vs-other-ingestion distinction is enforced via the API key's `metadata.allowedHostIds` field, not via a separate key type.

**Host-id binding (security boundary).** Each INGESTION key may carry a `metadata.allowedHostIds: string[]` provisioned at setup time. When configured, the receive endpoint enforces `dto.host_id ∈ apiKeyData.metadata.allowedHostIds` and returns `403 host_id not authorized` on miss. This protects against a stolen ingestion key being used to ingest fake data under another machine's identity. Keys WITHOUT `allowedHostIds` are legacy log-ingestion keys (no agent-gateway scope); the agent-gateway endpoint requires the allowlist to be set.
...
- Empty `watermarks: []` for a host with no captures is the success case, not an error. Treat it as "server has no cursor; start from 0". **Never returns 404 for "no watermarks"** — the empty array distinguishes from auth failure / endpoint disabled.
...
- Read-only (no write side effects); uses the existing per-API-key throttler (§9); reuses the `(user_id, host_id)` index that already exists for monotonicity checks. Realistic upper bound is 50–500 rows per host.
...
```jsonc
{
  ...
  "host_id":             "8a3aed6b9c1fdeadbeefcafef00d1234567890abcdef0123456789abcdef0123",
  "source_app":          "claude-code",
...
| `host_id` | string | yes | 64-char lowercase hex (`^[a-f0-9]{64}$`); must be in API key's `allowedHostIds` when set | Lowercase-hex sha256 of `machine_uuid + ':' + user_id` (see §5.4). Same physical machine + same user = same `host_id` across reinstalls and key rotations. |
...
**The one exception: watermark monotonicity violations.** When a 400 is caused by `watermark.start < server's last_processed_watermark_end` for the batch's `(host_id, source_path_hash, watermark_table)`, the response body is structured and gateway-recoverable. See §3.4 for the exact body shape.

**Gateway action on 400:**

- If the body's top-level `error` is `"watermark_regression"`: read `current_server_watermark_end`, write it to the local cursor for that source, drop the rejected batch, continue from the corrected position on the next poll. Self-healing — no operator intervention.
...
The backend stores it verbatim and uses it for parser dispatch. The receive DTO bounds the value: `MaxLength(128)` and `^[\w.+:\-]+$` (alphanumeric plus `._+:-`). Anything outside that returns `400`. If a parser becomes incompatible with a particular `agent_schema_version`, the backend handles that on its side; you don't need to coordinate.
...
| `400 Bad Request` (structured: `error: "watermark_regression"`) | Watermark monotonicity violated. Body carries `current_server_watermark_end` for the rejected `(host_id, source_path_hash, watermark_table)`. | **Self-heal:** write the server value to the local cursor, drop the rejected batch, retry from the corrected position with a NEW `capture_id` on the next poll. No operator intervention. See §3.4 / §6.3. |
| `400 Bad Request` (generic) | DTO validation failed, or table out of scope. Generic error message. | Mark batch `failed`. Surface in `proxai-gateway status`. **Do not retry, do not advance.** |
| `403 Forbidden` | Auth failed: ingestion key missing / malformed / revoked (state INACTIVE) / wrong type, OR `dto.host_id` not in `metadata.allowedHostIds`. | Treat as **retriable** (key-fix recoverable): drop a `~/.proxai/AUTH_FAILED` sentinel, keep batches pending, surface as auth error. Operator must rotate the key (re-run `setup`) or add the host_id to the allowlist. **The gateway does NOT mark the batch failed** — once auth is fixed, batches retry and succeed. |
...
### 8.3 Buffer-full and sentinel files

If the backend is unreachable for a sustained period, the local upload buffer fills. The gateway uses a **hysteresis-based** sentinel — `~/.proxai/BUFFER_FULL` — driven by config fields `bufferSoftPauseBytes` (default 700 MB) and `bufferSoftResumeBytes` (default 600 MB). When pending bytes cross the pause threshold the sentinel is created and capture pauses; when they fall back below the resume threshold the sentinel is removed and capture resumes. The watermark stays put while paused. This protects the user's disk from runaway gateway storage.

There are three sentinel files in `~/.proxai/`, each with a distinct role:

| Sentinel | Set by | Cleared by | Effect |
|---|---|---|---|
| `PAUSED` | User (`proxai-gateway pause`) | User (`proxai-gateway resume`) | Capture + upload paused, watermark frozen. |
| `BUFFER_FULL` | Daemon when pending bytes ≥ `bufferSoftPauseBytes` | Daemon when pending bytes drop below `bufferSoftResumeBytes` | Capture paused (upload continues to drain). |
| `AUTH_FAILED` | Daemon on `403` from `POST /v1/raw_records` | Daemon on the next successful upload | Surfaces auth error in `status`; batches stay pending. |
...
**Symptom:** `400 Bad Request` with `error: "watermark_regression"` body; backend metric `watermark_regression_total` increments.
**Cause:** sent a batch with `watermark.start < previous batch's watermark.end` for the same `(host_id, source_path_hash, watermark.table)`.
**Fix:** the gateway self-heals from the structured 400 body (§3.4) — write `current_server_watermark_end` to the local cursor, drop the rejected batch, retry with a NEW `capture_id`. For sources that are *genuinely* non-monotonic (e.g. a snapshot system that resets offsets), change `source_path` (e.g. add `#gen=2` suffix) so the `source_path_hash` differs.
...
### 12.7 Auth failure (wrong key type, revoked key, host_id not in allowlist)

**Symptom:** every upload returns `403`; `~/.proxai/AUTH_FAILED` sentinel surfaces in `proxai-gateway status`.
**Causes:**

- User supplied a SERVICE-type or PROVISIONING-type API key instead of an INGESTION key during setup.
- The INGESTION key was revoked after setup (`state: INACTIVE`).
- The key's `metadata.allowedHostIds` doesn't include this machine's `host_id` (e.g. user added a second machine and forgot to update the allowlist).
- The gateway changed `host_id` derivation (e.g. machine UUID rotated).

**Fix:** create or rotate an INGESTION key in the dashboard, ensure the current `host_id` is in `metadata.allowedHostIds`, and re-run `proxai-gateway setup`. The backend re-reads metadata on every request, so no full key rotation is needed for an allowlist update. Existing pending batches retry automatically once auth is fixed — the gateway treats 403 as retriable for exactly this reason and keeps the `AUTH_FAILED` sentinel until a successful upload clears it.
...
- `watermark.start` for this `(host_id, source_path_hash, watermark.table)` is less than the highest known `watermark.end` — **structured body, recoverable** (see §3.4)
...
The 64-char-lowercase-hex `host_id` rule also applies to the `host_id` query parameter on `GET /v1/watermarks`; mismatches return `400` there too.

### 15.1 What the backend rejects with 403

- API key missing / malformed / revoked (`state: INACTIVE`) / wrong type (e.g. SERVICE or PROVISIONING when INGESTION is required)
- `dto.host_id` not in `metadata.allowedHostIds` for this API key (operator must add the host_id; treat as retriable per §8.1)
...
- §1 expanded to cover three endpoints (`GET /ingestion/verify-key`, `POST /v1/raw_records`, `GET /v1/watermarks`).
- §2 corrected: auth header is `X-API-Key` (not `Authorization: Bearer`); the API key is `INGESTION` type with `metadata.allowedHostIds` as the security boundary (there is no `AGENT_GATEWAY` key type).
- §2.1 added: `GET /ingestion/verify-key` setup-time spec.
- §2.3 added: `GET /v1/watermarks` cold-start sync endpoint.
- §3.4 added: structured `400 watermark_regression` body shape and recovery path.
- §5.4 added: stable `host_id` derivation (`sha256(machine_uuid + ':' + user_id)`).
- §8.1 refined: 400 split into structured (recoverable) vs generic; 403 reclassified as retriable.
- §8.3 expanded: hysteresis-based `BUFFER_FULL` sentinel + `PAUSED` and `AUTH_FAILED` sentinels.
- §10 corrected: gateway runs single-pass redaction at capture time (not two stages).
- §15.1 added: 403 reject paths.
