...
├── services/                (buffer, polling, uploader, redaction, contract, config, http, upgrade, uninstall, state-machines, prompt-extract, rescue)
└── sources/                 (claude-code, cursor, codex, claude-desktop, gemini)
```
...
- `services/` is each long-running subsystem in the daemon: the SQLite buffer, the polling (capture) cycle, the uploader (drain) cycle, the redaction pipeline, the backend contract types, the config loader, the HTTP wrapper, the upgrade engine, the uninstall engine, the prompt-extraction utility (used by logs and tail), the rescue helper (crashed daemon recovery), and the state-machine-driven actors and coordination loops.
...
| --- | --- | --- | --- | --- | --- |
| **macOS** | `prod` | `~/Library/Application Support/proxai/proxai-gateway/prod/` | `~/Library/Logs/proxai/proxai-gateway/prod/` | `~/Library/Application Support/proxai/proxai-gateway/prod/control.sock` | `co.proxai.gateway.plist` |
| | `dev` | `~/Library/Application Support/proxai/proxai-gateway/dev/` | `~/Library/Logs/proxai/proxai-gateway/dev/` | `~/Library/Application Support/proxai/proxai-gateway/dev/control.sock` | `co.proxai.gateway.dev.plist` |
| **Linux** | `prod` | `~/.config/proxai/proxai-gateway/prod/` | `~/.local/state/proxai/proxai-gateway/log/prod/` | `~/.config/proxai/proxai-gateway/prod/control.sock` | `proxai-gateway.service` |
| | `dev` | `~/.config/proxai/proxai-gateway/dev/` | `~/.local/state/proxai/proxai-gateway/log/dev/` | `~/.config/proxai/proxai-gateway/dev/control.sock` | `proxai-gateway-dev.service` |
