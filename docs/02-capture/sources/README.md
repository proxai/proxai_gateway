...
| Gemini CLI | [gemini-cli.md](./gemini-cli.md) | `jsonl` chats with a leading header line | Every chat file under `<project>/chats/**/`, filtered to `gemini` records and text-bearing `user` records; each kept `gemini` record is trimmed | `PROXAI_GATEWAY_CAPTURE_SUB_AGENTS_GEMINI_CLI` is wired but currently no-op (no on-disk sub-agent artifacts exist). |
