...
flowchart TD
  ...
  subgraph Redact ["2. Redaction Pipeline"]
    redact["applyRedaction"]
  end

  subgraph Storage ["3. Buffering & Storage"]
    encode["Encode Redacted Text"] --> zstd["zstdCompressSync"] --> insert["insertBatch"]
  end
...
One field is deliberately **never** run through the rule pipeline: `source_path`. It is the absolute host path of the captured file and travels on the wire unredacted (see `build-dto.ts`), because the server keys watermarks on `source_path_hash` and any rule that rewrote a path string would corrupt watermark routing. The redaction rule contract forbids adding a pattern that matches path strings.

## How does redaction relate to the stored user prompt?

The redacted body is also where the gateway's locally-retained user prompt comes from. After a batch uploads successfully, `extractUserPrompt` (`services/prompt-extract/`) decompresses that same already-redacted body, pulls out the user-turn text, and `markBatchDelivered` writes it to the `user_prompt` column of the `upload_receipts` row (with `user_prompt_added_at`). Because extraction runs on the post-redaction body, the stored `user_prompt` is the **redacted** prompt — secrets are already stripped before the prompt is ever persisted for local log auditing. Agent responses are not stored; only the user prompt is. The receipt feeds the [logs command](../06-operations/6.3-cli-and-observability.md) and the durable receipt retention covered in [3.1 Buffer](../03-buffer/3.1-buffer.md). Extraction is best-effort and fail-soft: any decode or parse error yields `user_prompt = null` and the receipt is still written.
