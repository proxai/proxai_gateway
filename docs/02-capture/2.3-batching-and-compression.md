...
Each row stores not just the compressed body but the full identifying tuple: `source_app`, `source_kind`, `source_path`, `source_path_hash`, optional `source_inode`, the watermark fields, `agent_schema_version` (reported by the source), `gateway_version`, `captured_at_utc`, `body_format`, `body_compression`, plus drain-side bookkeeping (`status`, `attempts`, `last_error`, `created_at`). The full schema is in `BATCH_COLS` and gets covered field-by-field in the FAQs below.
