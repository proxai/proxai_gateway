[← Previous: 6.3 CLI & Observability](../06-operations/6.3-cli-and-observability.md) · [Index](../README.md) · [Next: 7.2 Install, Upgrade & Uninstall →](./7.2-install-upgrade-uninstall.md)
...
flowchart LR
  cli[start / stop / restart] --> wire[buildPlatformServiceContext]
  wire --> darwin[launchctl manager<br/>+ plist writer]
  wire --> linux[systemctl --user manager<br/>+ unit writer]
  wire --> win[schtasks manager<br/>+ XML writer]
  darwin --> plist[~/Library/LaunchAgents/<br/>co.proxai.gateway.plist]
  linux --> unit[~/.config/systemd/user/<br/>proxai-gateway.service]
  win --> xml[%LOCALAPPDATA%/proxai/proxai-gateway/<br/>scheduled-task.xml]
```
