...
| --- | --- | --- | --- |
| Profile root (config + buffer + sentinels) | `~/Library/Application Support/proxai/proxai-gateway/<profile>/` | `~/.config/proxai/proxai-gateway/<profile>/` | `%LOCALAPPDATA%\proxai\proxai-gateway\<profile>\` |
...
| Control IPC | `~/Library/Application Support/proxai/proxai-gateway/<profile>/control.sock` (Unix socket) | `~/.config/proxai/proxai-gateway/<profile>/control.sock` (Unix socket) | `\\.\pipe\proxai-gateway-control-<profile>` (named pipe) |
| Root markers (cross-profile) | `~/Library/Application Support/proxai/proxai-gateway/` | `~/.config/proxai/proxai-gateway/` | `%LOCALAPPDATA%\proxai\proxai-gateway\` |
| Binary install (curl/PS1) | `~/.proxai/bin/proxai-gateway` | `~/.proxai/bin/proxai-gateway` | `%USERPROFILE%\.proxai\bin\proxai-gateway.exe` |

`profileRootDir()` returns `<HOME>/Library/Application Support/proxai/proxai-gateway/` on darwin, `<HOME>/.config/proxai/proxai-gateway/` on linux, and `<LOCALAPPDATA>/proxai/proxai-gateway/` on Windows; `buildProfileContext(profile).configDir` joins the `<profile>` segment onto it. `profileLogDirRoot()` returns `<HOME>/Library/Logs/proxai/proxai-gateway/` on darwin, `<HOME>/.local/state/proxai/proxai-gateway/log/` on linux, and `<LOCALAPPDATA>/proxai/proxai-gateway/Logs/` on Windows; `logDir` likewise joins the `<profile>` segment.
