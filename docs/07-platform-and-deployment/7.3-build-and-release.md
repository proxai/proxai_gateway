...
```mermaid
%%{init: {'theme': 'base', 'flowchart': {'useMaxWidth': false, 'htmlLabels': true, 'curve': 'basis', 'nodeSpacing': 50, 'rankSpacing': 50, 'padding': 15}, 'themeVariables': {'background': '#081612', 'primaryColor': '#081612', 'primaryTextColor': '#e0fdf5', 'primaryBorderColor': '#10b981', 'lineColor': '#10b981', 'secondaryColor': '#0a201b', 'tertiaryColor': '#081612', 'fontSize': '14px'}}}%%
flowchart TD
  subgraph LocalDev ["Local Development & Build"]
    src["src/main.ts"] --> build["scripts/build.ts"]
    build --> dist["dist/[target]/proxai-gateway[.exe]"]
  end

  subgraph TagFlow ["Release Tagging"]
    cli["scripts/release/cli.ts"] --> tag["git tag v[calver]"]
  end

  subgraph CI ["CI/CD Pipeline (GitHub Actions)"]
    ghw["release.yml workflow"]
    npmpub["npm publish<br/>@proxai/gateway"]
    assets["Release assets<br/>renamed for npm convention"]
  end

  subgraph Dist ["Distribution Vectors"]
    release["GitHub Release"]
    curlbash["install.sh / install.ps1"]
    npmpost["npm postinstall"]
  end
  ...
  class src startNode;
  class cli startNode;
  class build processNode;
  class tag processNode;
  class ghw processNode;
  class dist actionNode;
  class assets actionNode;
  class curlbash actionNode;
  class npmpost actionNode;
  class release stopNode;
...
| `scripts/release/publish.ts` | The pure orchestration logic: clean-tree check, branch check, sync check, validate run, version compute, version bump/commit, tag create, tag and branch push. Wholly DI-driven so tests can inject mock git ops. |
...
`checksums.txt` is generated by `sha256sum` over the renamed files in `release/`. The action used to attach the assets is `softprops/action-gh-release@v3` with `make_latest: 'true'` and `fail_on_unmatched_files: true`.
...
8. **Update package.json and commit.** Update the version field in `package.json`, stage the file, and commit it with message `chore: release <version>`.
9. **Create and push tag & branch.** Create an annotated tag, push the `main` branch to origin, and push the tag to origin.
...
The release CLI never publishes the npm package or creates the GitHub Release itself — pushing the tag is what triggers the CI release workflow, which does the actual artifact build and release publication. This decoupling is deliberate: a developer can dry-run safely. Same-day retries are handled automatically by the versioning math, which adds or increments a `-N` suffix.
...
- Auto-generated release notes (the `generate_release_notes: true` option on `softprops/action-gh-release@v3` pulls commits since the previous tag).
