...
flowchart LR
  ...
  check -->|no| done([end])
  check -->|yes| brew{install_source<br/>== brew?}
  brew -->|yes| sentinel[Write UPDATE_AVAILABLE sentinel<br/>Operator runs brew upgrade]
  brew -->|no| download[runAutoUpgrade<br/>download asset]
  download --> replace[replaceBinary]
  replace --> posix{platform?}
  posix -->|POSIX| exit[exitProcess<br/>service mgr respawns]
  posix -->|Windows| stage[Stage at .new<br/>Operator must restart]
```
...
The stale-binary policy (covered in [heartbeat](../04-daemon-loops/4.3-heartbeat-cycle.md) and [sentinels](../04-daemon-loops/4.4-sentinels.md)) ramps in two stages: at 30 days since install, `tail --level warn` surfaces `stale_binary.warning` events; at 60 days, `checkStaleBinary` writes the `PAUSED` sentinel with reason `stale_binary: N days since install (>= 60)` and all three daemon cycles skip on the next tick.

Recovery requires both an upgraded binary **and** a manual `resume`: `setup --force` deliberately preserves the prior `installedAt` so a key rotation does not silently reset the clock, and an in-place auto-upgrade does not clear stale-binary `PAUSED` on its own. The only way to push the clock forward is `uninstall --reset` followed by a fresh `setup`.
...
- `daysSinceInstall >= pauseAfterDays` → write the `PAUSED` sentinel with reason `stale_binary: <N> days since install (>= <pauseAfter>)`. All three cycles will skip on the next ticks. Recovery requires both an upgraded binary and a manual `proxai-gateway resume`: `runSetup` deliberately preserves the prior `installedAt` on `setup --force`, so neither the upgrade nor a key-rotation `setup` resets the staleness clock. The only way to push the clock forward is `uninstall --reset` followed by a fresh `setup`, which records a new `installedAt = now()`.

A zero threshold disables that level. `installedAt` is the ISO-8601 timestamp recorded on first `setup`; it is preserved across `setup --force` re-runs so a key rotation does not silently reset the staleness clock — but as a consequence, in-place auto-upgrades do not clear stale-binary PAUSED on their own either.
