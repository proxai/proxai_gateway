...
2. **Dev daemon teardown** (if a dev service manager is wired): `devServiceManager.stop()` and `.unregister()` (best effort), then delete the dev unit file.
3. **Prod daemon teardown**: `serviceManager.stop()` then `.unregister()` (best effort), then delete the prod unit file.
4. If `--reset`: `rmRecursive` on prod `configDir`, prod `logDir`, dev `configDir`, dev `logDir`, then remove the shared root markers `.migrated-flat-to-nested`, `.upgrade-restore-state`, `.upgrade.lock`, `.migration.lock`, `DEV_MODE`.
