...
### Dev-Mode URL Switching and Sentinel Override

By default, the base URL defaults to the production endpoint: `https://proxainest-production.up.railway.app`.

The base URL can be controlled and overridden using two mechanisms:
1. **The `DEV_MODE` Sentinel File**: If a file named `DEV_MODE` exists in the gateway's platform-specific configuration directory, the base URL is automatically forced to the local development server at `http://localhost:3001`.
   - **Sentinel Paths**:
     - **macOS/Linux**: `~/.proxai/proxai-gateway/DEV_MODE`
     - **Windows**: `%LOCALAPPDATA%\proxai\proxai-gateway\DEV_MODE`
   - **Sentinel Content**: The file typically contains the text `ENABLED`.
   - **CLI Management**: Instead of creating this file manually, you can manage it using the CLI `dev` command:
     - `proxai-gateway dev on` — Forces the ingestion endpoints to `http://localhost:3001` by creating the `DEV_MODE` sentinel.
     - `proxai-gateway dev off` — Restores production ingestion endpoints by deleting the `DEV_MODE` sentinel.
     - `proxai-gateway dev` (no action) — Toggles the dev mode on/off based on the current presence of the sentinel file.
2. **Configuration file `[backend]` table overrides**: Individual endpoint keys under the `[backend]` table in `config.toml` always take highest precedence if they are explicitly configured:
...
```mermaid
%%{init: { 'flowchart': { 'useMaxWidth': false, 'curve': 'basis', 'nodeSpacing': 30, 'rankSpacing': 40, 'padding': 15 }, 'theme': 'base', 'themeVariables': { 'background': '#081612', 'primaryColor': '#081612', 'primaryTextColor': '#ffffff', 'primaryBorderColor': '#00ffc4', 'lineColor': '#00ffc4', 'secondaryColor': '#0c251f', 'tertiaryColor': '#081612' } }}%%
flowchart TD
  %% Native classDef Declarations
  ...
  subgraph Success ["Success & Recovery"]
    acc["UploadResult (accepted + idempotent)"]
    rec["WatermarkRegressionError (recovered)"]
  end

  subgraph Transient ["Transient Retriable Errors"]
    rl["RateLimitError (retriable)"]
    ret["RetriableError (retriable)"]
  end

  subgraph Fatal ["Terminal Fatal Errors"]
    val["ValidationError (fatal)"]
    auth["AuthError (verifyKey disambiguator)"]
    fatal["FatalError (fatal)"]
  end

  resp -->|"200/201"| acc
  resp -->|"400 watermark_regression"| rec
  resp -->|"429"| rl
  resp -->|"5xx"| ret
  resp -->|"400/408/413 other"| val
  resp -->|"401/403"| auth
  resp -->|"other"| fatal
