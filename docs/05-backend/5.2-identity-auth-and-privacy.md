...
| --- | --- |
| `POST /v1/raw_records` | The redacted compressed body, the watermark metadata, the source identity (including the absolute `source_path` — note that this is not redacted by the on-device rule set — and `source_app`, `source_platform`, and `source_inode`), and the host context (`host_id`, `gateway_version`, `agent_schema_version`, `captured_at_utc`) |
