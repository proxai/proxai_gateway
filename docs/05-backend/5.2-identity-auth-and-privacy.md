...
sequenceDiagram
  ...
  participant Server as ProxAI ingest
  CLI->>Server: GET /ingestion/verify-key (X-API-Key header)
  Server-->>CLI: { success, data: { userId, keyName } }
  Note over CLI: derive host_id from<br/>sha256(machine_uuid + ":" + userId)
  CLI->>Server: POST /v1/host-ids/register { host_id }
  Server-->>CLI: { host_id, user_id, registered }
```
...
The API key is stored exactly once: in the `[account]` table of `~/.proxai/proxai-gateway/config.toml`, written via `writeAtomic` and chmod'd to `0o600` (owner read/write only). It is transmitted exactly once per request, as the `X-API-Key` HTTP header. It is never logged, never serialized into a sentinel body, never embedded in an error message — `tail` and `status` never print it, and the only code path that handles it is the `HttpClient` constructor.

## The privacy posture

The privacy posture is "what crosses the wire is exactly what the drain cycle ships, nothing more."
...
**Does not see**: anything redacted before insert; anything from sources not in the supported set; anything outside the watched directories; anything outside the initial-scan window; the raw machine UUID; the boot id; pending/failed batches that are still local; receipts of older deliveries (those are local observability state, not re-sent).
