...
```mermaid
%%{init: {"flowchart": {"useMaxWidth": false, "htmlLabels": true, "curve": "basis", "nodeSpacing": 60, "rankSpacing": 80, "padding": 20}, "theme": "base", "themeVariables": {"background": "#081612", "primaryColor": "#0c231f", "primaryTextColor": "#e6f4f1", "primaryBorderColor": "#14b8a6", "lineColor": "#14b8a6", "secondaryColor": "#0d2d26", "tertiaryColor": "#081612", "mainBkg": "#0a201b", "mainBkgColor": "#0a201b", "mainBorderColor": "#14b8a6", "mainTextColor": "#ccfbf1"}, "themeCSS": ".edgePath .path { stroke: #14b8a6 !important; stroke-width: 2px !important; } .edgePath .marker-end { fill: #14b8a6 !important; } .label { color: #ccfbf1 !important; }"}}%%
flowchart TD
  ...
  init([Daemon Start])
  checkSentinel{"BUFFER_FULL sentinel exists?"}

  subgraph NormalSub["Normal Operation Mode"]
    stateNormal["Normal State<br/>• Capture runs normally<br/>• Drain ships pending batches"]
    checkPause{"Pending Bytes > 50 GiB?"}
    writeSentinel["Write BUFFER_FULL sentinel"]
  end

  subgraph PausedSub["Soft-Paused Operation Mode"]
    statePaused["Soft-Paused State<br/>• Capture checks sentinel & skips<br/>• Drain ships pending batches"]
    checkResume{"Pending Bytes < 45 GiB?"}
    clearSentinel["Clear BUFFER_FULL sentinel"]
  end

  init --> checkSentinel
  
  checkSentinel -->|No| stateNormal
  checkSentinel -->|Yes| statePaused
  
  stateNormal --> checkPause
  checkPause -->|No| stateNormal
  checkPause -->|Yes| writeSentinel
  writeSentinel --> statePaused
  
  statePaused --> checkResume
  checkResume -->|No| statePaused
  checkResume -->|Yes| clearSentinel
