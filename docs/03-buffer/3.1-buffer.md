...
| `source_app` | `TEXT` | `NOT NULL` | | Source application name (`claude-code`, `cursor`, `claude-desktop`, `codex`). |
| `source_platform` | `TEXT` | `NULL` | | Operating system platform on which the source ran. |
...
| `failed_at` | `TEXT` | `NULL` | | ISO-8601 UTC timestamp indicating when the batch was marked failed. |
...
- `migrateBatchFailedAtColumn(db)` — adds `failed_at` to `upload_batches`.
- `migrateBatchSourcePlatformColumn(db)` — adds `source_platform` to `upload_batches`.
