...
```mermaid
%%{init: {"flowchart": {"useMaxWidth": false, "htmlLabels": true, "curve": "basis", "nodeSpacing": 60, "rankSpacing": 80, "padding": 20}, "theme": "base", "themeVariables": {"background": "#081612", "primaryColor": "#0c231f", "primaryTextColor": "#e6f4f1", "primaryBorderColor": "#00ffc4", "lineColor": "#00ffc4", "secondaryColor": "#0d2d26", "tertiaryColor": "#081612", "mainBkg": "#0c231f", "mainBkgColor": "#0c231f", "mainBorderColor": "#00ffc4", "mainTextColor": "#e6f4f1"}, "themeCSS": ".edgePath .path { stroke: #00ffc4 !important; stroke-width: 2px !important; filter: drop-shadow(0 0 3px #00ffc4); } .edgePath .marker-end { fill: #00ffc4 !important; } .label { color: #e6f4f1 !important; }"}}%%
flowchart TD
  subgraph Capture ["Capture Cycle (Write Half)"]
  ...
  subgraph Drain ["Drain Cycle (Read Half)"]
    http{POST /v1/raw_records}
    recv[markBatchDelivered<br/>txn: insert receipt + delete batch]
    ur[(upload_receipts)]
  ...
  %% Decision Outcomes (dotted lines)
  http -.->|Retriable| retry
  http -.->|Fatal| failed
  http -.->|Recovered| del

  %% Theme Taxonomy Class Definitions
...
Concurrency is handled by SQLite's own file-level locking. The daemon is single-process and runs the three loops ([capture](../04-daemon-loops/4.1-capture-cycle.md), [drain](../04-daemon-loops/4.2-drain-cycle.md), [heartbeat](../04-daemon-loops/4.3-heartbeat-cycle.md)) in the same Bun process under `Promise.all`, so within-process writes are serialized by SQLite. Cross-process reads (CLI commands like `status`) open the database via `openBufferDb` (which opens read-write) but because SQLite is configured in WAL mode, these concurrent readers never block the daemon's write loop. The install pipeline registers exactly one service unit per host so two daemons never race against the same `buffer.db`.
...
The daemon is single-process. All three loops (capture, drain, heartbeat) run in the same Bun process via `Promise.all` in `runDaemonLoops`, and SQLite serialises writes through the WAL. Reads from the CLI (`status`, `tail`) open the same file via `openBufferDb` (which opens read-write under the hood); WAL allows concurrent readers without blocking the daemon's writes.
...
The schema is additive: every DDL is `CREATE TABLE IF NOT EXISTS` and every index is `CREATE INDEX IF NOT EXISTS`. The only explicit migrations are the two `ALTER TABLE ... ADD COLUMN` calls in `migrateCursorVacuumColumns(db)`, guarded by `columnExists(db, 'source_cursors', '...')`, and the addition of `machine_snapshots` to the `daemon_state` table in `migrateDaemonStateMachineSnapshots(db)` guarded by `columnExists(db, 'daemon_state', 'machine_snapshots')`. There is no `schema_version` table — `IF NOT EXISTS` makes the open-time DDL idempotent.
