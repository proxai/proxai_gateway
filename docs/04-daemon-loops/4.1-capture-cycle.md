...
```mermaid
%%{init: { 'flowchart': { 'useMaxWidth': false, 'htmlLabels': true, 'curve': 'basis', 'nodeSpacing': 50, 'rankSpacing': 50, 'padding': 15 }, 'theme': 'base', 'themeVariables': { 'background': '#070f19', 'primaryColor': '#0d1b2d', 'primaryTextColor': '#bfdbfe', 'primaryBorderColor': '#3b82f6', 'lineColor': '#38bdf8', 'secondaryColor': '#1e112c', 'tertiaryColor': '#0a201b', 'fontSize': '14px' } }}%%
flowchart TD
    start([runCaptureCycle]) --> gate{sentinel gates}

    subgraph SkipPaths ["Skip Pathways (Short-Circuit)"]
        direction LR
        skip1["skip: auth_failed"]
        skip2["skip: buffer_full"]
    end

    subgraph ActivePath ["Active Capture Pathway"]
        direction TD
        poll["Poll sources in order:<br/>claude-code, cursor, codex, gemini-cli"]
        post["Post-cycle bookkeeping:<br/>pressure sentinel + metrics + state"]
        poll --> post
    end

    gate -->|AUTH_FAILED| skip1
    gate -->|BUFFER_FULL| skip2
    gate -->|clear| poll
