...
> One of the three daemon loops — walks every source, decides what's new, buffers it.

The capture cycle is the loop that pulls new bytes from every supported [source](../02-capture/2.1-sources.md) and writes them into the [buffer](../03-buffer/3.1-buffer.md). It runs every 2 minutes (`CAPTURE_INTERVAL_MS = 120_000`) inside the gateway daemon, in parallel with the [drain cycle](./4.2-drain-cycle.md) and [heartbeat cycle](./4.3-heartbeat-cycle.md) under `Promise.all` in `runDaemonLoops`. The 2-minute cadence is low enough to feel near-real-time yet high enough that idle hosts barely consume CPU.
...
flowchart TD
  start([runCaptureCycle])
  start --> gate{sentinel gates}
  gate -->|AUTH_FAILED| skip1[skip: auth_failed]
  gate -->|PAUSED| skip2[skip: paused]
  gate -->|BUFFER_FULL| skip3[skip: buffer_full]
  gate -->|clear| poll[Poll sources in order:<br/>claude-code, cursor, codex, gemini-cli]
  poll --> post[Post-cycle bookkeeping:<br/>pressure sentinel + metrics + state]
  post --> done([CaptureCycleResult])
```

Each cycle has a fixed three-phase shape: gate, poll, post-cycle bookkeeping. The gate phase checks three [sentinels](./4.4-sentinels.md) in order — `AUTH_FAILED`, then `PAUSED`, then `BUFFER_FULL` — and short-circuits with a `capture.cycle.skipped` log line on the first hit.

The ordering matters: auth-failed is the most absorbing state (only an operator can clear it), paused is a deliberate user action, buffer-full is a transient self-healing condition. A skipped cycle returns flags on its result and does not increment `capture_cycles_total`, so the metadata counter reflects only completed cycles.

## Concurrent worker-based polling

To maximize efficiency and keep CPU consumption on the primary thread minimal, the poll phase executes all registered default sources (`claude-code`, `cursor`, `codex`, `gemini-cli`) concurrently in background Bun Worker threads (`services/polling/poll-worker.ts`) using `Promise.all`. For custom or mock sources, it cleanly falls back to direct main-thread linear polling.

Each default source is executed inside its own isolated worker process:
1. **Prior Cursors Fetching**: The main thread reads all existing cursors for the source from `source_cursors` in `buffer.db` and passes them to the spawned Bun Worker.
2. **Worker Isolation**: The worker is spawned under type `module` and initializes a dedicated temporary in-memory database (`:memory:`). It populates this memory DB with the prior cursors.
...
A thrown error inside `source.poll(ctx)` propagates out — `runCaptureCycle` does **not** wrap individual polls in try/catch. The throw bubbles to `captureLoop` in `daemon-loops.ts`, which catches and logs `event: capture.cycle.error`, then sleeps and tries again on the next tick.
...
The capture loop runs in parallel with drain and heartbeat under `Promise.all` in `runDaemonLoops`, so a long capture cycle does not block drains.

## What gates does the cycle check before doing work?

Three sentinel checks at the top of `runCaptureCycle`, in this order:
...
2. `isPaused(pauseSentinelPath)` — returns early with `reason: paused`. Cleared by `proxai-gateway resume`.
3. `isBufferFull(bufferFullSentinelPath)` — returns early with `reason: buffer_full`. Cleared by the drain cycle once pending bytes drop below the resume threshold.

A skipped cycle still logs `event: capture.cycle.skipped` with the reason, still returns a result with `paused` / `authFailed` / `bufferFull` flags set, and still consumes its interval — the daemon sleeps and tries again on the next tick.

## Are sources polled in parallel?

Yes. The four default sources (`claude-code`, `cursor`, `codex`, `gemini-cli`) are polled in parallel using concurrent Bun Worker threads via `Promise.all` inside the capture cycle. Spawning these workers concurrently ensures that slower or larger sources (e.g. Cursor snapshotting or large Codex rollouts) never block cheaper sources (e.g. small JSONL appends in Claude Code or Gemini CLI).
...
Every poller follows the same scaffold (`makeClaudeCodeSourcePoller`, `makeCodexSourcePoller`, `makeCursorSourcePoller`, `makeGeminiCliSourcePoller`):
...
sequenceDiagram
  ...
  participant W4 as Bun Worker (gemini-cli)
  C->>C: INFO capture.cycle.start
  Note over C,W4: Spawn workers concurrently via Promise.all
  ...
  C-->>W4: postMessage(task: capture, priorCursors)
  Note over W1,W4: Background polling & in-memory SQLite buffering
  ...
  C->>C: Write cursor updates, quarantine, and batches under ACID transaction
  W4-->>C: onmessage(success, captureResult)
  C->>C: Write cursor updates, quarantine, and batches under ACID transaction
...
DEBUG source.poll.start                   source_app=gemini-cli
...
INFO  source.poll.complete                source_app=gemini-cli files=0 batches=0 bytes=0 errors=0
...
No. For backwards compatibility and extensibility, custom or mock sources are executed linearly on the main thread as a fallback. Only the four default core sources (`claude-code`, `cursor`, `codex`, `gemini-cli`) utilize the Bun Worker multi-threaded infrastructure.
