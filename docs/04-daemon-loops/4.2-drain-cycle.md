...
flowchart TD
  ...
  subgraph Cycle ["Active Drain & Bookkeeping Cycle"]
    loop[Drain loop:<br/>FIFO batches, up to 256]
    prune[pruneBuffer:<br/>receipts + failed + quarantined]
    resume[applyResumeSentinel:<br/>maybe clear BUFFER_FULL]
    persist[persistDaemonState +<br/>persistDrainMetrics]
  end

  done([DrainCycleResult])
...
flowchart TD
  ...
  subgraph RecoveredBranch ["Recovered Path"]
    recovered[setCursorFromRegression<br/>deleteBatch<br/>kind = recovered]
  end
