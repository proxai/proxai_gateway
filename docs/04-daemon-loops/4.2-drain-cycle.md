...
flowchart LR
  start([runDrainCycle])
  start --> gate{AUTH_FAILED?<br/>PAUSED?}
...
Each cycle has a five-phase shape: gate, drain loop, prune, resume, persist. The gate checks `AUTH_FAILED` and `PAUSED` only — `BUFFER_FULL` does **not** gate drain because drain is precisely the loop that relieves buffer-full pressure.
...
Two sentinel checks, both via the same paths the capture cycle uses:
...
2. `isPaused` — returns early with `reason: paused`. PAUSED halts shipping the same way it halts capture.
