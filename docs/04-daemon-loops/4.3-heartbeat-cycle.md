...
flowchart TD
  ...
  start --> gate{PAUSED?}
  gate -->|yes| skip[skip: paused]
  gate -->|no| stale[checkStaleBinary<br/>every tick]
  stale --> rate{4h since last<br/>version check?}
  rate -->|no| done([complete])
  rate -->|yes| brew{install_source<br/>== brew?}
  brew -->|yes| sentinel[runBrewSentinelCheck<br/>write UPDATE_AVAILABLE]
  brew -->|no| upgrade[runAutoUpgrade<br/>download + replaceBinary]
  sentinel --> done
  upgrade --> done
```

**Stale-binary detection** is a local-only timestamp subtraction against `installedAt` from the account config, with two thresholds: `DEFAULT_STALE_WARN_DAYS = 30` and `DEFAULT_STALE_PAUSE_DAYS = 60`. At 30 days the cycle logs `event: stale_binary.warning`; at 60 days it calls `pausePolling` to write the `PAUSED` sentinel with reason `stale_binary: N days since install (>= 60)`. Both thresholds are zero-disableable in `[stale_binary]` of `config.toml`. The stale check is **not** gated by the 4-hour version-check interval — it runs every heartbeat tick because the operator might have already done the upgrade between two hourly checks, and firing more often surfaces the cleared state faster.
...
%%{init: {'flowchart': {'useMaxWidth': false, 'htmlLabels': true, 'curve': 'basis', 'nodeSpacing': 60, 'rankSpacing': 80, 'padding': 20}, 'sequence': {'useMaxWidth': false, 'mirrorActors': false, 'actorMargin': 100, 'messageMargin': 50, 'boxMargin': 20, 'wrap': true}, 'stateDiagram': {'useMaxWidth': false, 'nodeSpacing': 60, 'rankSpacing': 80}, 'themeVariables': {'fontSize': '15px'}}}%%
flowchart TD
  ctx[heartbeat ctx] --> dev{devMode?}
  dev -->|yes| skip[short-circuit, no upgrade]
  dev -->|no| brew{installSource == brew?}
  brew -->|yes| sentinelPath{updateAvailableSentinelPath set?}
  sentinelPath -->|yes| sentinelWrite[runBrewSentinelCheck<br/>writes UPDATE_AVAILABLE]
  sentinelPath -->|no| skip
  brew -->|no| inPlace{binaryPath +<br/>currentVersion both set?}
  inPlace -->|yes| download[runAutoUpgrade<br/>download + replaceBinary +<br/>exitProcess on success]
  inPlace -->|no| skip
```
...
%%{init: {'flowchart': {'useMaxWidth': false, 'htmlLabels': true, 'curve': 'basis', 'nodeSpacing': 60, 'rankSpacing': 80, 'padding': 20}, 'sequence': {'useMaxWidth': false, 'mirrorActors': false, 'actorMargin': 100, 'messageMargin': 50, 'boxMargin': 20, 'wrap': true}, 'stateDiagram': {'useMaxWidth': false, 'nodeSpacing': 60, 'rankSpacing': 80}, 'themeVariables': {'fontSize': '15px'}}}%%
flowchart TD
  start[heartbeat tick] --> read[read installedAt from config]
  read --> compute[daysSinceInstall = now - installedAt]
  compute --> pause{>= pauseAfterDays<br/>(default 60)?}
  pause -->|yes| writePause[pausePolling writes PAUSED<br/>reason='stale_binary: N days'']
  pause -->|no| warn{>= warnAfterDays<br/>(default 30)?}
  warn -->|yes| logwarn[log stale_binary.warning]
  warn -->|no| done([no action])
  writePause --> done
  logwarn --> done
```
...
- `DEFAULT_STALE_PAUSE_DAYS = 60` — when `daysSinceInstall >= 60`, call `pausePolling(pauseSentinelPath, …)` to write the PAUSED sentinel with reason `stale_binary: N days since install (>= 60)`. The next capture and drain cycles will skip via the PAUSED gate; only an upgrade clears the situation.

Both defaults are overridable in `config.toml` under `[stale_binary]`. A zero value disables that threshold (`pauseAfterDays = 0` means never pause for staleness; `warnAfterDays = 0` means never warn).
