...
The daemon and the CLI commands (`pause`, `resume`, `setup`, `uninstall`) run in different processes, so they cannot share in-memory flags. Sentinel files are the lowest-overhead way to coordinate between them without a database round-trip and without a watch-based notification mechanism.

## The six sentinels

Six sentinels exist. The first three are the operationally important ones; the others are observability and lifecycle markers.

| sentinel | role |
| --- | --- |
| **`PAUSED`** | operator pause or stale-binary trip; halts capture, drain, and heartbeat |
...
| **`SESSION_STOPPED`** | daemon shutdown handler recorded the current `boot_id`; gates the next `start` |
...
`writeAtomic` writes to a temp file in the same directory and renames into place, so a `kill -9` mid-write leaves either the previous body or the new body, never a torn one. Every cycle calls the matching `is*` helper at the top of its run before any work — five existence checks per tick across three cycles is fifteen `stat`s total, single-digit microseconds on a healthy host.
...
flowchart LR
  writer[Writer] --> file[(sentinel file)]
  file --> observer[Observer<br/>gates loop]
  clearer[Clearer] --> file
```
...
- `PAUSED` is written by the `pause` command (and by the [heartbeat cycle](./4.3-heartbeat-cycle.md)'s stale-binary policy past 60 days), observed by all three cycles, cleared by the `resume` command — the daemon never clears `PAUSED` on its own.
...
- `PAUSED` carries an arbitrary reason string.
...
The `SESSION_STOPPED` sentinel deserves a closer look because it is the only sentinel whose read is self-clearing. It is written by the daemon's shutdown handler with the **current** boot id, and the next-start gate compares the stored boot id against the live boot id via `isCurrentSessionStopped(path, currentBootId)`. If they match, the gate fires and `start` refuses to launch. If they differ — the host has rebooted since the daemon was stopped — the sentinel is silently cleared and `start` proceeds.
...
stateDiagram-v2
  direction LR
  [*] --> NoSentinel
  NoSentinel --> Set: shutdown handler writes\n{bootId: <current>}
  Set --> Refused: start re-checks; bootId matches\n(daemon was stopped in THIS boot)
  Set --> Cleared: start re-checks; bootId differs\n(reboot has occurred)
  Cleared --> NoSentinel: file unlinked
  Refused --> [*]: gate denies launch
```

The boot id source is `/proc/sys/kernel/random/boot_id` on Linux, `sysctl kern.boottime` (sha256'd) on macOS, and `LastBootUpTime` (sha256'd) on Windows.

## Sentinel lifecycle reference

| sentinel | writer | observer (gate) | clearer |
| --- | --- | --- | --- |
| `PAUSED` | `pause` cmd; heartbeat's `checkStaleBinary` | `runCaptureCycle` (2nd check); `runDrainCycle` (2nd check); `runHeartbeatCycle` (only check) | `resume` cmd (daemon never clears it on its own) |
| `AUTH_FAILED` | uploader's `handleAuthError` | `runCaptureCycle` (1st check); `runDrainCycle` (1st check); heartbeat not gated | `setup --force` after verify-key succeeds |
| `BUFFER_FULL` | `runCaptureCycle` post-pressure | `runCaptureCycle` (3rd check); drain & heartbeat not gated | `runDrainCycle` if shouldResume && sentinel present |
| `SESSION_STOPPED` | daemon shutdown handler | `run/index.ts` pre-start via `isCurrentSessionStopped(bootId)`; status surfacing | a fresh boot (different boot_id self-clears) |
...
Six files, all under `~/.proxai/proxai-gateway/` (or the Windows equivalent):

| sentinel | trigger | cleared by | what it blocks |
| --- | --- | --- | --- |
| `PAUSED` | `proxai-gateway pause` command, or stale-binary policy past `pauseAfterDays` | `proxai-gateway resume` only — the daemon never removes PAUSED on its own. `setup --force` preserves the prior `installedAt`, so a binary upgrade alone does not push the stale-binary clock forward; recovery is `upgrade` plus `resume` plus optionally `uninstall --reset` then a fresh `setup` to actually reset `installedAt` | capture, drain, heartbeat |
...
| `SESSION_STOPPED` | written by run-mode shutdown handlers (and persists across the current OS session; payload contains `bootId` so reads of a stale stop from a previous boot self-clear) | a fresh boot, or `proxai-gateway start` | gates the next `start` until cleared (`run/index.ts` reads it via `isCurrentSessionStopped` before launching loops); surfaced in `status` |
...
`pause-sentinel.ts`, `auth-failed-sentinel.ts`, `buffer-full-sentinel.ts`, `session-stopped-sentinel.ts`, and `update-available-sentinel.ts` each export the `is*` / `write*` / `read*` / `clear*` helpers for the corresponding file.
...
`renderSentinelLines` (in `cli/commands/status/render-human.ts`) reads each sentinel's content and emits a colored line under the top "Status: …" header when any are active. AUTH_FAILED renders red with the reason and a "re-run setup" hint; PAUSED renders red with the reason and a "run resume" hint; BUFFER_FULL renders red with `(pending Nmb)`; SESSION_STOPPED renders red with the set time and a "restart" hint. UPDATE_AVAILABLE renders cyan at the bottom of the status output with "run `proxai-gateway upgrade`". If none are active, the Health section's `Sentinels` row prints `none active`.

In structured logs, the events are namespaced: `capture.cycle.skipped`, `drain.cycle.skipped`, `buffer.soft_pause`, `buffer.soft_resume`, `auth.invalid`, `stale_binary.paused`, etc. Filter `tail` by `--level info` for routine sentinel transitions; `--level warn` catches the back-pressure flips.
...
`checkStaleBinary` runs from the heartbeat cycle and reaches into the **PAUSED** sentinel directly — it does not have its own sentinel file. Two thresholds (defaults `30` warn, `60` pause):

1. `daysSinceInstall >= warnAfterDays` → log `event: stale_binary.warning`. No sentinel; the daemon keeps running.
2. `daysSinceInstall >= pauseAfterDays` → `pausePolling(pauseSentinelPath, "stale_binary: N days since install (>= 60)")`. The PAUSED sentinel is now set and capture/drain/heartbeat will skip on the next ticks. Only `proxai-gateway resume` removes it. Upgrading the binary alone does not clear the sentinel and does not reset the stale-binary clock — `runSetup` preserves the prior `installedAt` on `setup --force`. To restart the clock and clear the sentinel after recovery, run `uninstall --reset` then `setup` (which records a new `installedAt = now()`).
...
The gate order matters: `runCaptureCycle` checks AUTH_FAILED → PAUSED → BUFFER_FULL and returns on the first hit. So a buffer-full + paused + auth-failed daemon logs `reason: auth_failed` on the skip line until auth is fixed, then `reason: paused` until paused is cleared, then `reason: buffer_full` until drain catches up. `renderSentinelLines` displays **all** active sentinels — operators see the full overlapping state, even though the gate only fires on one.

The drain cycle similarly prefers AUTH_FAILED over PAUSED. BUFFER_FULL never gates drain so there's no third-tier choice there.

## What happens on a write race?

Two simultaneous writes to the same sentinel path are rare but possible: e.g. an operator runs `pause` while the heartbeat is mid-`pausePolling` from a stale-binary trip. Both calls go through `writeAtomic`, which writes to a fresh temp file and `rename`s it; the second rename overwrites the first. The body of the loser is lost; the winner's body wins. The gate observers see one sentinel either way.

Three classes of concurrent operation that **cannot** race because they operate on different paths:

- `pause` (PAUSED) vs heartbeat's `writeUpdateAvailableSentinel` (UPDATE_AVAILABLE).
- The drain's `clearBufferFullSentinel` vs the capture's `writeAuthFailedSentinel`.
- Setup's `clearAuthFailedSentinel` vs the uploader's `writeAuthFailedSentinel` — these touch the same path, but setup happens out-of-process from the daemon, so a setup re-run while a daemon is failing auth will lose to the next capture/drain tick that re-detects the failure and re-writes the sentinel.
