...
There are two profiles — `prod` and `dev` — each with its own config directory under the profile root (`~/.proxai/proxai-gateway/` on POSIX, `%LOCALAPPDATA%\proxai\proxai-gateway\` on Windows). `buildProfileContext(profile)` (`core/io/fs/profile.ts`) resolves `configDir = profileRootDir()/<profile>`, and `buildSentinelPaths(configDir)` places **five gate/observability sentinels inside each profile's own configDir**:

- `<root>/prod/{AUTH_FAILED, BUFFER_FULL, SESSION_STOPPED, CONSENT_ACCEPTED, UPDATE_AVAILABLE}`
- `<root>/dev/{AUTH_FAILED, BUFFER_FULL, SESSION_STOPPED, CONSENT_ACCEPTED, UPDATE_AVAILABLE}`

Each daemon reads **its own** profile's sentinels only — the prod daemon never inspects the dev profile's `AUTH_FAILED`, and vice versa. Both daemons can run simultaneously, each gated independently by its own files.
...
## The five per-profile sentinels

Five sentinels exist per profile. The first two are the operationally important loop gates; the others are observability and lifecycle markers.
...
Five per-profile files, each under that profile's `configDir` (`<root>/prod/` or `<root>/dev/`, where `<root>` is `~/.proxai/proxai-gateway/` or the Windows equivalent):
...
Like `SESSION_STOPPED`, `DEV_MODE` is **boot-scoped and self-clearing**: `readDevModeSentinel` (`core/io/fs/dev-mode-sentinel.ts`) parses the body, compares the stored `bootId` against `readBootId()`, and `rmSync`-removes the file (returning `false`) on a mismatch, a malformed body, or any read error. So a reboot silently turns dev mode off without an explicit `dev off`.
