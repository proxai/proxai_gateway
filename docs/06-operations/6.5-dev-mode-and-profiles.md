...
| `sentinels` | the five sentinel paths under `configDir` (`AUTH_FAILED`, `BUFFER_FULL`, `SESSION_STOPPED`, `CONSENT_ACCEPTED`, `UPDATE_AVAILABLE`) |
...
2. Read and `JSON.parse` the body. If it is malformed, or `bootId` is not a string → delete the file and return `false`.
3. Compare the stored `bootId` against `readBootId()` (`src/core/system/boot-id.ts`). If they differ → delete the file (self-clear) and return `false`.
...
- **The flag self-clears on reboot.** Because the stored boot id stops matching, after a reboot you are back in the prod perspective by default — even if the dev daemon's service unit brought the dev daemon back up. The daemon's lifecycle and the CLI's perspective are fully decoupled.
...
Older installs stored prod state *flat* at the config root — `config.toml`, `buffer.db`, and the sentinels directly under `~/.proxai/proxai-gateway/` with no `prod/` subdirectory. The dev-mode-isolation work nests everything under `<profile>/`, so legacy state has to move once.
