...
- `--lines <n>` — number of records to display (default `20`).
- `--error` — show only **failed + quarantined** records (the error view).
- `--pending` — show only **pending** (queued, not-yet-uploaded) records.
- `--source <app>` — filter by `claude-code | cursor | codex | gemini-cli`.
...
The three views are mutually exclusive in `gatherLogsFrame` (`cli/commands/logs/gather-records.ts`): `--error` → `{failed, quarantined}`; `--pending` → `{pending}`; default → `{uploaded}`.
...
interface LogsQueryOptions {
  limit: number;        // from --lines, default 20
...
> Note: the `logs` `UploadedRecord` does **not** carry the redacted user prompt — that field lives in the **status** "Last Uploads" ledger (`LastUploadRow.userPrompt`, see [the status section](#what-does-status-show-in-each-section)). `logs` deliberately keeps the uploaded view terse: time, source, and the re-sent marker.

### How are records mapped to displayed lines?

`render-logs.ts` maps each DTO to a single line. Brevity vs. detail is keyed on `deps.isDevMode` (resolved in `cli/wiring/logs-deps.ts` from the root `DEV_MODE` sentinel).

- **Uploaded** (`renderUploadedRow`):
  - **prod / non-god-mode**: `<local time> (<relative>)  <source>` + a dim ` (re-sent)` suffix when `idempotentOnServer` is true.
  - **dev / god-mode**: appends `<captureId>  hash:<first 8 of sourcePathHash>…` after the source.
- **Failed** (`renderFailedRow`): `<time> (<rel>)  <yellow source>  <dim truncated path>  attempts: N` plus the red `lastError` truncated to 80 chars when present.
- **Quarantined** (`renderQuarantinedRow`): `<time> (<rel>)  <magenta source>  <dim path>  <size>  <red reason truncated to 60>`.
- **Pending** (`renderPendingRow`): `<time> (<rel>)  <blue source>  <dim path>` plus ` attempts:N` only when `attempts > 0`.

Paths are truncated to 48 chars (`TRUNCATE_PATH`). Sections are printed under bold headers (`Uploaded` / yellow `Failed` / red `Quarantined` / blue `Pending`). When every view is empty the renderer prints a mode-specific dim message (`No uploaded records yet.` / `No errors found.` / `No pending records.`).
...
- `sentinels` — `authFailed`, `bufferFull`, `sessionStopped`, `updateAvailable` (existence checks via `sentinelHandle(path).exists()`).
- `buffer` — `pendingCount`, `pendingBytes`, `failedCount`, `quarantinedCount`, `receiptCount`, `lastPruneAt`, `lastSuccessAt`.
- `daemonState` — `captureLastCycleAt`, `drainLastCycleAt`, `lastConsecutiveRetriableBreak`.
...
- `filesystem` — `configDirWritable`, `logDirWritable`, `diskFreeBytes` (currently `null`).
- `network` — `nestReachable` (a 5 s `GET` to the verify-key URL; `false` on any throw, `null` when not probed).
- `sourcePaths` — existence of `~/.claude`, the Cursor config dir, `~/.codex`, `~/.config/gemini`.
...
- `platform`, `systemdLingerEnabled` (Linux `loginctl`), `macOsQuarantineXattr` (macOS `xattr`), `clockSkewMs` (currently `null`).
...
`runDoctor` runs `ALL_CHECKERS` (30 pure `(signals) => Finding | null` functions) and collects the non-null `Finding`s. Each `Finding` carries `{ code, severity, confidence, cause, action }` where severity ∈ `critical|warning|info|healthy` and confidence ∈ `CONFIRMED|LIKELY`. The catalog groups by prefix:

| group | checker file | covers |
| --- | --- | --- |
| **A — lifecycle** | `checkers/lifecycle.ts` | A1 not set up · A2 unit not registered · A3 stopped by user (SESSION_STOPPED) · A4 crashed (no sentinel) · A5 wedged (capture stale > 2× interval) |
| **B — auth** | `checkers/auth.ts` | B1 invalid key (AUTH_FAILED present) · B2 auth-unconfirmed loop (network, NOT a key problem) |
...
| **D — capture** | `checkers/capture.ts` | D1 no agent activity (info if dirs exist, warning if none) · D2 one source erroring |
| **E — binary** | `checkers/binary.ts` | E1 stale binary + upgrade failing · E2 brew update pending · E3 auto-upgrade write_failed (disk/perms) · E4 upgrade success but old binary still running |
...
| **G — data integrity** | `checkers/data-integrity.ts` | G1 receipts table readable · G2 buffer-db corrupt · G3 watermark regression loop |
...
- **B1 (key problem)** fires only when the **`AUTH_FAILED` sentinel is present**. That sentinel is written exclusively by `handleAuthError` after `verifyKey()` returns a definitive `{ success: false }` — i.e. the server rejected the key. Action: `setup --force` with a valid key.
...
[CONFIRMED] B1 AUTH_FAILED sentinel present — API key rejected by server
  → Run: proxai-gateway setup --force with a valid key; verify at proxai.co
...
- **Capture** — per-source `captured / files scanned / errors` from `daemonState.lastSourceCaptures`, fixed order `claude-code, cursor, codex, gemini-cli`.
