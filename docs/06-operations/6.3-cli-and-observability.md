...
- `--source <app>` — filter by `claude-code | cursor | codex | claude-desktop`.
...
- `sourcePaths` — existence of `~/.claude`, the Cursor config dir, `~/.codex`.
...
- **Capture** — per-source `captured / files scanned / errors` from `daemonState.lastSourceCaptures`, fixed order `claude-code, cursor, codex, claude-desktop`.
