...
- The CLI's exit codes are explicit and stable: 0 ok, 1 generic error, 2 validation, 3 auth, 4 not-installed, 5 already-installed, 7 file-unreadable, 130 user-aborted (Ctrl-C through an interactive prompt). Exit code 6 is intentionally skipped.
...
`main.ts` declares the full catalog. Visible commands and their aliases (from `command-aliases.ts`): `setup` (`init`), `start` (`s`), `stop` (`x`), `restart` (`r`), `status` (`i`), `inspect` (`ins`), `tail` (`t`), `uninstall` (`rm`). Visible without aliases: `pause`, `resume`, `redaction list`, `redaction test <file>`, `upgrade`. Hidden via `{ hidden: true }`: `run` (foreground daemon invoked by the service unit) and `dev` / `d` (foreground daemon with development defaults). The README's command table is the user-facing description; for internal purposes it suffices to know what each command's `index.ts` does — they are documented elsewhere in this series under their respective subsystem docs.
...
| `capture.cycle.skipped` | INFO | `reason` (`auth_failed` \| `paused` \| `buffer_full`) |
...
```
                          +-------------------------------+
                          |    proxai-gateway inspect    |
                          +---------------+---------------+
                                          |
                        1. Initialize Scanners (Concurrently)
                                          v
             +----------------------------+----------------------------+
             | (Parallel Dry-Run Worker Scans - Promise.all)           |
             v                                                         v
   +---------+---------+                                     +---------+---------+
   ...
   +---------+---------+                                     +---------+---------+
             |                                                           |
             | - Query SQLite DBs (readonly: true)                       | - Discover files
             | - Discover JSONL files                                    | - Parse line counts
             | - Parse without buffering                                 | - No redaction/commit
             v                                                           v
   +---------+---------+                                     +---------+---------+
   | Return Dry-Run    |                                     | Return Dry-Run    |
   | file & byte stats |                                     | file & byte stats |
   +---------+---------+                                     +---------+---------+
             |                                                           |
             | postMessage(dryRunResult)                                 | postMessage(dryRunResult)
             +----------------------------+------------------------------+
                                          |
                                          v (Gather results)
                      +-------------------+---------------+
                      | Aggregate Stats & local timezone  |
                      | oldest record timestamps          |
                      +-------------------+---------------+
                                          |
             +----------------------------+----------------------------+
             |                                                         |
             v (Print beautiful aligned table)                         v (Save MD Report)
     +-------+-------+                                         +-------+-------+
     |   Terminal    |                                         | `/tmp/` Report|
     |  Stdout View  |                                         |  Markdown file|
     +---------------+                                         +---------------+
```
