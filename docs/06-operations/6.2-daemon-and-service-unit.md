...
| **Linux** | `prod` | `~/.config/systemd/user/proxai-gateway.service`<br/>Service: `proxai-gateway.service` | systemd user service triggering `ExecStart=<programPath> run --profile prod`. `Type=simple`, `Restart=always`, `RestartSec=5s`, `StartLimitIntervalSec=300`, `StartLimitBurst=5`, `OOMScoreAdjust=-100` |
| | `dev` | `~/.config/systemd/user/proxai-gateway-dev.service`<br/>Service: `proxai-gateway-dev.service` | systemd user service triggering `ExecStart=<programPath> run --profile dev`. `Type=simple`, `Restart=always`, `RestartSec=5s`, `StartLimitIntervalSec=300`, `StartLimitBurst=5`, `OOMScoreAdjust=-100` |
...
### Rescue Decision Logic & Scenarios

When the watchdog timer triggers the `rescue` command, it executes the decision logic defined in [decideRescue](file:///Users/onurseckinsenoglu/repos/proxai/proxai_gateway/src/services/rescue/rescue-decision.ts#L37) to check the daemon's runtime state and heartbeat data. The watchdog categorizes its behavior into **automatic rescue** actions and **intentional stand-downs** (where user intervention is required first).

```mermaid
%%{init: {"theme": "dark", "themeVariables": {"primaryColor": "#0a201b", "primaryTextColor": "#ccfbf1", "primaryBorderColor": "#14b8a6", "lineColor": "#14b8a6", "fontFamily": "Inter, sans-serif"}, "flowchart": {"nodeSpacing": 35, "rankSpacing": 40}}}%%
flowchart TD
    StartRescue(["Watchdog Timer Fires"]) --> CheckConfig{Config Exists?}
    CheckConfig -->|No| SkipNoConfig["Skip (Not Configured)"]
    CheckConfig -->|Yes| CheckRegistered{Service Registered?}
    
    CheckRegistered -->|No| SkipNoRegister["Skip (Not Registered)"]
    CheckRegistered -->|Yes| CheckUserStopped{User Stopped?}
    
    CheckUserStopped -->|Yes| SkipUserStopped["Skip (User Stopped)"]
    CheckUserStopped -->|No| CheckCircuit{"Circuit Broken?<br/>(consecutiveFailures >= 3)"}
    
    CheckCircuit -->|Yes| SkipCircuitBroken["Skip (Circuit Broken)"]
    CheckCircuit -->|No| CheckRateCap{"Rate Capped?<br/>(lastRescue < 1 hour)"}
    
    CheckRateCap -->|Yes| SkipRateCapped["Skip (Rate Capped)"]
    CheckRateCap -->|No| CheckRunning{Is Daemon Running?}
    
    CheckRunning -->|No| CheckStaleDown{"Heartbeat Stale?<br/>(age > 10 min)"}
    CheckRunning -->|Yes| CheckSentinel{Auth Failed or Buffer Full?}
    
    CheckStaleDown -->|Yes| ActionStart["Action: Start Daemon"]
    CheckStaleDown -->|No| SkipHealthy["Skip (Healthy)"]
    
    CheckSentinel -->|Yes| SkipPaused["Skip (Paused)"]
    CheckSentinel -->|No| CheckStaleWedge{"Heartbeat Stale?<br/>(age > 30 min)"}
    
    CheckStaleWedge -->|Yes| ActionRestart["Action: Restart Daemon"]
    CheckStaleWedge -->|No| SkipHealthy
    ...
    class StartRescue startNode;
    class CheckConfig decNode;
    class CheckRegistered decNode;
    class CheckUserStopped decNode;
    class CheckCircuit decNode;
    class CheckRateCap decNode;
    class CheckRunning decNode;
    class CheckStaleDown decNode;
    class CheckSentinel decNode;
    class CheckStaleWedge decNode;
    class SkipNoConfig stopNode;
    class SkipNoRegister stopNode;
    class SkipUserStopped stopNode;
    class SkipCircuitBroken stopNode;
    class SkipRateCapped stopNode;
    class SkipHealthy stopNode;
    class SkipPaused stopNode;
    class ActionStart actionNode;
    class ActionRestart actionNode;
```


#### Automatic Rescue Scenarios
The watchdog automatically recovers the daemon under two conditions:
1. **Hard Crash (Not Running)**: If the daemon process is unexpectedly stopped or dead (`isRunning: false`), and either no heartbeat exists or the last heartbeat is older than 10 minutes (`SUSTAINED_DOWN_MS`).
   - **Action**: `{ kind: 'start' }` (kickstarts the service using the platform's service manager).
2. **Wedged/Frozen Daemon**: If the daemon process is running (`isRunning: true`), but the event loops are frozen and have failed to write heartbeats for more than 30 minutes (`WEDGE_STALE_MS`).
   - **Action**: `{ kind: 'restart' }` (terminates the wedged process and restarts it).

#### Intentional Stand-Down Scenarios (Manual User Resolution Required)
In the following situations, automatic rescue is skipped (`kind: 'none'`) because restarting the daemon cannot fix the root cause:
*   **Authentication Failure (`authFailedPresent`)**: An invalid or revoked gateway API key was detected. The daemon stays paused until the user runs `proxai-gateway setup` to supply a valid key.
*   **Buffer Storage Limit Reached (`bufferFullPresent`)**: The local buffer is full. Captures are paused until the uploader successfully drains the buffer (or the storage configuration is adjusted).
*   **Circuit Breaker Tripped (`consecutiveFailures >= 3`)**: The watchdog attempted 3 consecutive rescues but the daemon crashed immediately each time without logging healthy progress. This protects system resources from infinite crash-loops. The user must run `proxai-gateway doctor` to diagnose and resolve the environment issue.
*   **Explicit User Stop (`sessionStoppedThisBoot`)**: The user manually ran `proxai-gateway stop`. The watchdog respects this action and will not restart the daemon.
*   **Upgrade in Progress (`upgradeInProgress`)**: A binary upgrade is active.
*   **Unconfigured State (`configExists === false`)**: No `config.toml` exists; setup is incomplete.
