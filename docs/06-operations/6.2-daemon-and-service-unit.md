...
sequenceDiagram
  participant SM as Service manager<br/>(launchd / systemd / Task Scheduler)
  participant Bin as proxai-gateway run
  participant Loops as runDaemonLoops
  participant Buf as buffer.db
  Note over SM: user logs in
  SM->>Bin: spawn binary with 'run' arg
  Bin->>Bin: loadConfigFromFile + wire deps
  Bin->>Loops: Promise.all([capture, drain, heartbeat])
  Loops->>Buf: insert / drain / metrics
  Note over Loops: each loop runs on its own cadence
  SM-->>Bin: SIGTERM (stop or restart)
  Bin->>Bin: abort signal fires; loops exit
  Bin-->>SM: process exit 0
```
